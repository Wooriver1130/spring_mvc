...
// Controller servlet-context.xml로 리턴한다.
// RestController 그렇게 하지 않고 브라우저로 바로 출력한다.
@RestController
public class AjaxRestController {
	
// 반환형이 String인 경우 문서 타입이 자동으로 contentType="text/html"로 자동 변경시키자.
	// 텍스트 가져요기
	@RequestMapping(value = "/test01", produces="text/html; charset=utf-8")
	@ResponseBody
	public String Text_test01() {
	...
	// 공공데이터 api에서  xml을 가져오기
	@RequestMapping(value = "/test04", produces="text/xml; charset=utf-8")
	@ResponseBody
	public String xml_test04() {
		BufferedReader rd = null ;
		 HttpURLConnection conn = null ;
		try {
			   StringBuilder urlBuilder = new StringBuilder("http://apis.data.go.kr/B552584/ArpltnInforInqireSvc/getCtprvnRltmMesureDnsty"); /*URL*/
		        urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "=YTfivTrbMt2e9OVEpUVxjVVvkB0IS5UhWy0SNDytklK%2BQSKqogSFGjcdgC0jaeK112uCcXg4gZDZ7Oxbcfgh6w%3D%3D"); /* 본인의 서비스 키 = Service Key*/
		      ...
		      //  conn.setRequestProperty("Content-type", "application/json");
		        System.out.println("Response code: " + conn.getResponseCode()); // 200이면 ok
...
@ResponseBody
public String json_test07() {
	 BufferedReader rd = null ;
	 HttpURLConnection conn = null ;
	try {
		StringBuilder urlBuilder = new StringBuilder("http://apis.data.go.kr/1360000/MidFcstInfoService/getMidTa"); /*URL*/
        urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "=YTfivTrbMt2e9OVEpUVxjVVvkB0IS5UhWy0SNDytklK%2BQSKqogSFGjcdgC0jaeK112uCcXg4gZDZ7Oxbcfgh6w%3D%3D"); /*Service Key*/
        urlBuilder.append("&" + URLEncoder.encode("pageNo","UTF-8") + "=" + URLEncoder.encode("1", "UTF-8")); /*페이지번호*/
        urlBuilder.append("&" + URLEncoder.encode("numOfRows","UTF-8") + "=" + URLEncoder.encode("10", "UTF-8")); /*한 페이지 결과 수*/
